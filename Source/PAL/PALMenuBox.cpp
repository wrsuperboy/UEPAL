...
void UPALMenuBox::AddItemAndBind(const FString& Name, UObject* InObject, const FName& InFunctionName)
{
	...
	UTextBlock* ItemButtonText = WidgetTree->ConstructWidget<UTextBlock>(UTextBlock::StaticClass());
	ItemButtonText->SetText(FText::FromString(Name));
	ItemButtonText->SetFont(FontInfo);
	ItemButtonText->SetColorAndOpacity(FSlateColor::UseForeground());
	ItemButtonText->SetShadowOffset(FVector2D(1 * UI_PIXEL_TO_UNIT, 1 * UI_PIXEL_TO_UNIT));
	ItemButtonText->SetShadowColorAndOpacity(FLinearColor(0, 0, 0, 165.f / 255.f));
	UButtonSlot* ItemButtonTextSlot = Cast<UButtonSlot>(ItemButton->AddChild(ItemButtonText));
	ItemButtonTextSlot->SetPadding(FMargin(0, 1., 0, 1.) * UI_PIXEL_TO_UNIT);
	ItemButtonTextSlot->SetHorizontalAlignment(EHorizontalAlignment::HAlign_Center);
	ItemButtonTextSlot->SetVerticalAlignment(EVerticalAlignment::VAlign_Center);
	TScriptDelegate<> OnClick;
	OnClick.BindUFunction(InObject, InFunctionName);
	ItemButton->OnClicked.Add(OnClick);
	ItemList.Add(ItemButton);
	ItemButtonList.Add(ItemButton);
}

UPALMenuAnchor* UPALMenuBox::AddMenuItemWithBind(const FString& Name, UObject* InObject, const FName& InFunctionName)
{
	UPALCommon* Common = GetGameInstance()->GetSubsystem<UPALCommon>();
	FButtonStyle ButtonStyle;
	FSlateBrush SlatBrushNone;
	...
	ItemButtonTextSlot->SetHorizontalAlignment(EHorizontalAlignment::HAlign_Center);
	ItemButtonTextSlot->SetVerticalAlignment(EVerticalAlignment::VAlign_Center);
	UPALMenuAnchor* MenuAnchor = WidgetTree->ConstructWidget<UPALMenuAnchor>(UPALMenuAnchor::StaticClass());
	...
	ItemButton->OnClicked.Add(OnClick);
	ItemList.Add(MenuAnchor);
	ItemButtonList.Add(ItemButton);
	return MenuAnchor;
}

void UPALMenuBox::SetHittable(bool bHittable)
{
	if (bHittable)
	{
		for (UButton* ItemButton : ItemButtonList)
		{
...
TSharedRef<SWidget> UPALMenuBox::RebuildWidget()
{
...
void UPALMenuBox::NativeConstruct()
{
